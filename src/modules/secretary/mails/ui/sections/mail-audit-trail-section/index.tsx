...
import { MailAuditTrailSectionSkeleton } from './skeleton';
import { Card } from '@/components/ui/card';
import { useSuspenseInfiniteQuery } from '@tanstack/react-query';

interface MailAuditTrailSectionProps {
  mailId: string;
}

export const MailAuditTrailSection = ({
  mailId,
}: MailAuditTrailSectionProps) => {
  return (
    <Suspense fallback={<MailAuditTrailSectionSkeleton />}>
      <ErrorBoundary FallbackComponent={ErrorFallback}>
        <MailAuditTrailSuspense mailId={mailId} />
      </ErrorBoundary>
    </Suspense>
  );
};

export function MailAuditTrailSuspense({ mailId }: { mailId: string }) {
  const trpc = useTRPC();
